...
Details are provided bellow, after the example.
...
1. For each test-suite found in `twister.json`:
   1. Create output directory `package/<test-suite>/<board>/`.
   2. Determine which files to copy for this test-suite from `east.yml`.
   3. Copy the files and rename them according to the twister artifact naming scheme (See bellow).
2. If any extra files are specified in `east.yml`:
   ...
   2. For each extra file:
      ...
      2. Rename the file according to the extra artifact naming scheme (See bellow).
3. Zip top level directories in `package/`:
...
extra artifacts - the specification can be found [bellow](#artifact-naming-scheme).

To simplify:

- Twister artifacts will be renamed to `<test-suite>-<filename>-<version>.<ext>`.
...
- `file` = scripts/update_docker_version.sh
- `version_str` = "v1.0.0"
