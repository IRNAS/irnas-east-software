...
There are a few rules that are related to the build types and the way they are specified in
`east.yml`:
...
To configure an application in `east.yml`, the user must specify which `west-boards` and
