...
def _pack(east, twister_out_path: str, pack_path: str, tag: str):
    ...
    # All possible checks succeded, time to do some filesystem operations.
...
def check_for_failed_testsuites(testsuites: Sequence[TSuite]):
    """Check if any of the testsuites failed, and raise an error if so."""
    failed_testsuites = [ts for ts in testsuites if ts.did_fail()]
...
def check_that_all_artifacts_exist(east, artifacts: Sequence[Artifact]):
    ...
    header += "The following build projets are missing atleast one file:\n"
