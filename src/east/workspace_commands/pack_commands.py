...
@click.option(
    ...
    type=str,
    default="twister-out",
    help=("Path to the twister-out directory from which to copy files from. Default: [bold cyan]twister-out[/]."),
)
@click.option(
    "--tag",
    type=str,
    ...
    "If this argument is provided, east assumes that the git HEAD "
    "is on the tagged commit and that the repo is clean. "
    "If it is not provided, the tag will be determined using git describe.",
)
@click.option(
    "-p",
    "--pack-path",
...
def pack(east, twister_out_path: str, pack_path: str, tag: str, verbose: bool):
    ...
    \b
    \n\nCopy and rename artifacts from the [bold cyan]twister-out[/] directory and other extra files
    specified in [bold yellow]east.yml[/] to the [bold cyan]pack-path[/] directory.
...
def check_that_specific_projects_exist_as_twister_artifacts(
    ...
    """Check if all specified projects with additional or overwritten artifacts exist inside the twister-out directory.
